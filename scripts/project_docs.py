...
``.project-docs.lock.json``. The implementation uses only the Python standard
...
def extract_index_blocks(text):
    """Extract top-level project-docs fences while respecting longer fences."""
    blocks, active = [], None
    capture = None
    for line in text.splitlines():
        ...
        if active is None:
            ...
            capture = [] if info == "project-docs-index" else None
            continue
...
class Project:
    ...
    def _load_lock(self):
        path = self.root / ".project-docs.lock.json"
        if not path.exists():
            return
        try:
            ...
            self.lock_error = Issue("LOCK_CORRUPTED", f"rebuild with verify: {exc}", ".project-docs.lock.json")
...
class ProjectDocsArgumentParser(argparse.ArgumentParser):
    def error(self, message):
        raise CliArgumentError(message)


def build_parser():
    parser = ProjectDocsArgumentParser(description="Project Docs v2 graph and freshness manager")
    subs = parser.add_subparsers(dest="command", required=True, parser_class=ProjectDocsArgumentParser)
